...
const [addFormData, setAddFormData] = useState({
  ...
  items: [{ itemId: null, name: "", quantity: "", price: 0 }],
});
    ...
    const fetchItems = async () => {
      try {
        ...
        const normalized = arr.map((i: any) => ({
          ...
          price: Number(
            i.price ?? i.default_price ?? i.unit_price ?? i.defaultPrice ?? 0
          ),
        }));
        setAvailableItems(normalized);
        const pricingMap: Record<string, number> = {};
        normalized.forEach(
          (it: any) => (pricingMap[it.name] = Number(it.price) || 0)
        );
  ...
  const removeItemRow = (index: number) => {
    if (addFormData.items.length > 1) {
      setAddFormData({
        ...addFormData,
        items: addFormData.items.filter((_, i) => i !== index),
      });
    }
  };
...
const updateItemRow = (index: number, field: string, value: any) => {
  ...
  if (field === "itemId") {
    const selected = availableItems.find((it) => String(it.id) === value);
    newItems[index] = {
      ...newItems[index],
  ...
  const handleAddInventoryRequest = async () => {
   const validItems = addFormData.items.filter(
  (i) => i.itemId && i.name && i.quantity && Number(i.price) > 0
);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-start">
        <p className="text-muted-foreground">Kitchen inventory requests</p>
  ...
  onValueChange={(v) => updateItemRow(index, "itemId", v)}
>
  <SelectTrigger>
      ...
      <Dialog open={showViewDialog} onOpenChange={setShowViewDialog}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Request for {viewingRequest ? formatDate(viewingRequest.date) : ""}
            </DialogDescription>
          </DialogHeader>
          {viewingRequest && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    {formatDate(viewingRequest.date)}
                  </p>
                </div>
                <div>
