...
export function Dashboard({
  ...
  const handleProfileClick = () => {
    console.log("Profile clicked"); // For debugging
    setActiveModule("profile");
  };

  const handleLogoutClick = () => {
    console.log("Logout clicked from dropdown"); // For debugging
    onLogout();
  };
  ...
  const renderActiveModule = () => {
    switch (activeModule) {
      ...
      case "inventory-request":
        return <InventoryRequestModule />;
      case "stock-availability":
        return <StockAvailabilityModule />;
      case "users":
        return <UsersModule />;
      case "finance":
  ...
  return (
    <div className="h-screen bg-background flex flex-col">
      {/* Fixed Header */}
      <header className="fixed top-0 left-0 right-0 z-50 border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 h-16 flex items-center justify-between px-6">
        {/* Logo */}
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 bg-primary rounded flex items-center justify-center text-primary-foreground">
            {currentUser?.logo ? (
              <img src={currentUser.logo} alt="Company Logo" className="w-full h-full object-contain rounded" />
            ) : (
      ...
      <div className="flex pt-16 h-screen">
        {/* Fixed Sidebar */}
        <aside
          className={`fixed left-0 top-16 bottom-0 z-40 border-r bg-sidebar transition-all duration-300 ${
